...
/* get meterological parameters 获取卫星气象参数----------------------------------------------*/
static void getmet(double lat, double *met)
{
    ...
    };//各行前五个为不同维度范围下各个气象参数的年平均值；后五个为不同纬度范围下各个气象参数的季节变化值
    int i,j;
    double a;
    lat=fabs(lat);
    if      (lat<=15.0) for (i=0;i<10;i++) met[i]=metprm[0][i];
        ...
        for (i=0;i<10;i++) met[i]=(1.0-a)*metprm[j-1][i]+a*metprm[j][i];//相当于插值运算
    }
}
                          ...
                          double *var)//详情请参考EGNOS模型
{
    ...
    if (zh==0.0||fabs(pos[0]-pos_[0])>1E-7||fabs(pos[1]-pos_[1])>1E-7||
        ...
        getmet(pos[0]*R2D,met);//获取卫星气象参数
        c=cos(2.0*PI*(time2doy(time)-(pos[0]>=0.0?28.0:211.0))/365.25);
        for (i=0;i<5;i++) met[i]-=met[i+5]*c;//计算平均海平面气象参数，met[i]为平均海平面气象参数
    ...
    m=1.001/sqrt(0.002001+sinel*sinel);//加入卫星方位角考虑因素
    *var=0.12*0.12*m*m;
    return (zh+zw)*m;
}
...
*          double 
ar      O   sat position and clock variance (m^2)
* return : status (1:ok,0:no correction)
