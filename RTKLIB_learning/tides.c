...
#ifndef IERS_MODEL
static void tide_pl(const double *eu, const double *rp, double GMp,//GMp--the parameter for the Moon or the Sun
                    const double *pos, double *dr)//pos为站点的经度和纬度；
{
    ...
    for (i=0;i<3;i++) ep[i]=rp[i]/r;//the unit vector of the Moon or the Sun--ep
    
    K2=GMp/GME*SQR(RE_WGS84)*SQR(RE_WGS84)/(r*r*r);
    K3=K2*RE_WGS84/r;
    latp=asin(ep[2]); lonp=atan2(ep[1],ep[0]);//太阳或者月球的赤经赤纬；
                       ...
                       double *dr)//pos为站点的经度和纬度；
{
    ...
    /* step1: time domain */
    eu[0]=E[2]; eu[1]=E[5]; eu[2]=E[8];//站点在地固坐标下的方向余弦
...
static void tide_oload(gtime_t tut, const double *odisp, double *denu)
{
    ...
    /* angular argument: see subroutine arg.f for reference [1] 角参数*/
    time2epoch(tut,ep);
    fday=ep[3]*3600.0+ep[4]*60.0+ep[5];//时分秒化为秒
    ep[3]=ep[4]=ep[5]=0.0;
    ...
    a[0]=fday;
    a[1]=(279.69668+36000.768930485*t+3.03E-4*t2)*D2R; /* H0 mean longitude of sun at begining of day*/
    a[2]=(270.434358+481267.88314137*t-0.001133*t2+1.9E-6*t3)*D2R; /* S0 mean longitude of moon at begining of day*/
    a[3]=(334.329653+4069.0340329577*t-0.010325*t2-1.2E-5*t3)*D2R; /* P0 mean longitude of lunar perigee(近地点) at begining of day*/
    a[4]=2.0*PI;
...
/* tidal displacement ----------------------------------------------------------
* displacements by earth tides 固体潮位移
* args   : gtime_t tutc     I   time in utc
*          double *rr       I   site position (ecef) (m)
*          int    opt       I   options (or of the followings)
*                                 1: solid earth tide	固体潮
*                                 2: ocean tide loading 海潮负荷
*                                 4: pole tide			极潮
*                                 8: elimate permanent deformation永久变形估计
*          double *erp      I   earth rotation parameters (NULL: not used)地球自转参数
*          double *odisp    I   ocean loading parameters  (NULL: not used)海洋负荷参数
                     ...
                     const double *odisp, double *dr)
{
    ...
    if (erp) {
        geterp(erp,utc2gpst(tutc),erpv);//获得地球自转参数
    }
    tut=timeadd(tutc,erpv[2]);//加入世界时与协调世界时之差改正后的接收机时间
    
    dr[0]=dr[1]=dr[2]=0.0;//潮汐改正数初始化
    
    if (norm(rr,3)<=0.0) return;//
    
    pos[0]=asin(rr[2]/norm(rr,3));//获站点得纬度
    pos[1]=atan2(rr[1],rr[0]);//获得站点经度
    xyz2enu(pos,E);//获得地固坐标向站心坐标的转换的矩阵E
    
    if (opt&1) { /* solid earth tides */
        
        /* sun and moon position in ecef */
        sunmoonpos(tutc,erpv,rs,rm,&gmst);//获得太阳与月亮地固坐标系下的坐标
...
#else
        tide_solid(rs,rm,pos,E,gmst,opt,drt);//drt太阳月球引起的固体潮坐标改正；
#endif
        for (i=0;i<3;i++) dr[i]+=drt[i];
    }
    if ((opt&2)&&odisp) { /* ocean tide loading */
        tide_oload(tut,odisp,denu);//存疑
