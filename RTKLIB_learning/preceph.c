...
//针对原来的读取部分sp3文件的头文件不适用的情况，重写一个读物sp3头文件的函数------李伟
static int readsp3h_new(FILE *fp, gtime_t *time, char *type, int *sats, double *bfact, char *tsys)
{
	...
	while (fgets(buff, sizeof(buff), fp))
	{
		...
		if (i == 2)  ns = (int)str2num(buff, 3, 3);     //卫星数量
		if (!strncmp(buff, "+ ", 2)) {
			for (j = 0; j < 17 && k<ns; j++) {
		...
		if (!strncmp(buff, "%c", 2) && !strlen(tsys)) {
			strncpy(tsys, buff + 9, 3); tsys[3] = '\0';  //时间系统
		}
		if (!strncmp(buff, "%f", 2) && bfact[0] * bfact[1] < 1.0e-10) {
                    ...
                    double *bfact, char *tsys)
{
    ...
    for (i=0;i<22;i++) {//the first 22 rows
        if (!fgets(buff,sizeof(buff),fp)) break;//fgets()--read the string and put it into the buff according to row.
        
        if (i==0) {
            *type=buff[2];//position identifier
            if (str2time(buff,3,28,time)) return 0;
        }
        else if (2<=i&&i<=6) {
            if (i==2) {
                ns=(int)str2num(buff,4,2);//total number of satellite
            }
            for (j=0;j<17&&k<ns;j++) {
                sys=code2sys(buff[9+3*j]);//satellite code to satellite system????I don't understand 
                prn=(int)str2num(buff,10+3*j,2);
                if (k<MAXSAT) sats[k++]=satno(sys,prn);//return prn+sys+1 value to sats
            }
        }
        else if (i==12) {
            ...
            bfact[0]=str2num(buff, 3,10);//1.25
            bfact[1]=str2num(buff,14,12);//1.05
        }
    }
    return ns;
}
                     ...
                     char *tsys, int index, int opt, nav_t *nav)
{
    ...
    while (fgets(buff,sizeof(buff),fp)) {
        ...
        for (i=0;i<MAXSAT;i++) {
            ...
            for (j=0;j<3;j++) {
                peph.cov[i][j]=0.0f;// satellite position covariance 
                peph.vco[i][j]=0.0f;//satellite velocity covariance
            }
        }
        for (i=pred_o=pred_c=v=0;i<n&&fgets(buff,sizeof(buff),fp);i++) {
            ...
            sys=buff[1]==' '?SYS_GPS:code2sys(buff[1]);//position identifier
            prn=(int)str2num(buff,2,2);//number of satellite
            ...
            for (j=0;j<4;j++) {
                ...
                std=str2num(buff,61+j* 3,j<3?2:3);//DOP of x
                
                if (buff[0]=='P') { /* position */
                    if (val!=0.0&&fabs(val-999999.999999)>=1E-6) {
                        peph.pos[sat-1][j]=val*(j<3?1000.0:1E-6);//position unit is km;clock bias unit is 1E-6s
                        v=1; /* valid epoch */
                    }
                    if ((base=bfact[j<3?0:1])>0.0&&std>0.0) {
                        peph.std[sat-1][j]=(float)(pow(base,std)*(j<3?1E-3:1E-12));//satellite poition /clock std
                    }
                }
                else if (v) { /* velocity */
                    if (val!=0.0&&fabs(val-999999.999999)>=1E-6) {
...
extern void readsp3(const char *file, nav_t *nav, int opt)
{
    ...
    for (i=0;i<MAXEXFILE;i++) {
        if (!(efiles[i]=(char *)malloc(1024))) {//allocte memory space for efiles[];
    ...
    for (i=j=0;i<n;i++) {
        ...
        if (!(fp=fopen(efiles[i],"r"))) {//only read the file
            trace(2,"sp3 file open error %s\n",efiles[i]);
            continue;
        }
        /* read sp3 header */
        //ns=readsp3h(fp,&time,&type,sats,bfact,tsys);
		ns = readsp3h_new(fp, &time, &type, sats, bfact, tsys);
