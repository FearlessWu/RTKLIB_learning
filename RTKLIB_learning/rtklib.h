...
#define SC2RAD      3.1415926535898     /* semi-circle to radian (IS-GPS)圆周率 */
#define AU          149597870691.0      /* 1 AU (m) 天文距离*/
#define AS2R        (D2R/3600.0)        /* arc sec to radian 秒转弧度*/

#define OMGE        7.2921151467E-5     /* earth angular velocity (IS-GPS) (rad/s)地球角速度 */
...
#define MAXFREQ     7                   /* max NFREQ 最大频率数目*/
//各导航系统的频率数
...
//各系统的时间系统
#define TSYS_GPS    0                   /* time system: GPS time  时间系统 */
...
#ifndef NFREQ
#define NFREQ       3                   /*条件编译 number of carrier frequencies 载波频率*/
#endif
#define NFREQGLO    2                   /* number of carrier frequencies of GLONASS 格洛纳斯载波频率*/

#ifndef NEXOBS
#define NEXOBS      0                   /* number of extended obs codes 增加扩展的观测码类型数量*/
#endif

#define MINPRNGPS   1                   /* min satellite PRN  伪随机噪声码 number of GPS */
#define MAXPRNGPS   32                  /* max satellite PRN number of GPS */
#define NSATGPS     (MAXPRNGPS-MINPRNGPS+1) /* number of GPS satellites GPS卫星数目 */
#define NSYSGPS     1

#define ENAGLO
#ifdef ENAGLO
#define MINPRNGLO   1                   /* min satellite slot跟踪 位置 number of GLONASS */
#define MAXPRNGLO   27                  /* max satellite slot number of GLONASS */
#define NSATGLO     (MAXPRNGLO-MINPRNGLO+1) /* number of GLONASS satellites 格洛纳斯卫星数目*/
#define NSYSGLO     1
#else
#define MINPRNGLO   0
#define MAXPRNGLO   24
...
#define ENAGAL
#ifdef ENAGAL
#define MINPRNGAL   1                   /* min satellite PRN number of Galileo 伽利略卫星*/
...
#endif
#ifdef ENAQZS
#define MINPRNQZS   193                 /* min satellite PRN number最少伪随机噪声码 of QZSS准天顶卫星系统 */
...
#define ENACMP
#ifdef ENACMP
#define MINPRNCMP   1                   /* min satellite sat number of BeiDou 北斗 */
...
#endif
#ifdef ENAIRN
#define MINPRNIRN   1                   /* min satellite sat number of IRNSS 印度区域卫星系统*/
...
#define MINPRNSBS   120                 /* min satellite PRN number of SBAS星基增强系统最少伪随机码数目 */
#define MAXPRNSBS   142                 /* max satellite PRN number of SBAS */
#define NSATSBS     (MAXPRNSBS-MINPRNSBS+1) /* number of SBAS satellites */

#define MAXSAT      (NSATGPS+NSATGLO+NSATGAL+NSATQZS+NSATCMP+NSATIRN+NSATSBS+NSATLEO)
                                        /* max satellite number (1 to MAXSAT) 最大卫星数目*/
#define MAXSTA      255                 // ??? 
//接收机部分
#ifndef MAXOBS
#define MAXOBS      64                  /* max number of obs in an epoch 每一个历元最大观测数目 */
#endif
#define MAXRCV      64                  /* max receiver number 最多接收机数目(1 to MAXRCV) */
#define MAXOBSTYPE  64                  /* max number of obs type in RINEX 最大观测类型数目*/
#define DTTOL       0.005               /* tolerance of time difference时间限差阈值 (s) */
...
#define MAXGDOP     300.0               /* max GDOP 最大几何精度衰减因子*/

#define INT_SWAP_TRAC 86400.0           /* swap interval of trace file (s)轨道文件的交换间隔 */
#define INT_SWAP_STAT 86400.0           /* swap interval of solution status file 解决状态文件交换间隔(s) */

#define MAXEXFILE   1024                /* max number of expanded files 最多延伸文件数目*/
#define MAXSBSAGEF  30.0                /* max age of SBAS fast correction 修正(s) */
#define MAXSBSAGEL  1800.0              /* max age of SBAS long term corr更正 (s) */
#define MAXSBSURA   8                   /* max URA 用户距离误差 of SBAS satellite */
#define MAXBAND     10                  /* max SBAS band of IGP */
#define MAXNIGP     201                 /* max number of IGP in SBAS band */
#define MAXNGEO     4                   /* max number of GEO satellites 最大的地球静止卫星*/
...
#define MAXSOLMSG   8191                /* max length of solution message 解算消息的最长长度*/
...
#define OBSTYPE_PR  0x01                /* observation type: pseudorange 伪距*/
#define OBSTYPE_CP  0x02                /* observation type: carrier-phase 载波相位*/
#define OBSTYPE_DOP 0x04                /* observation type: doppler-freq 多普勒频率*/
...
#define PMODE_SINGLE 0                  /* positioning mode: single /单点定位*/
#define PMODE_DGPS   1                  /* positioning mode: DGPS/DGNSS /差分！！！ */
#define PMODE_KINEMA 2                  /* positioning mode: kinematic /动态定位模式*/
#define PMODE_STATIC 3                  /* positioning mode: static /静态定位模式*/
#define PMODE_MOVEB  4                  /* positioning mode: moving-base /移动基线*/
#define PMODE_FIXED  5                  /* positioning mode: fixed /定点*/
#define PMODE_PPP_KINEMA 6              /* positioning mode: PPP-kinemaric /ppp动态*/
#define PMODE_PPP_STATIC 7              /* positioning mode: PPP-static /ppp静态*/
#define PMODE_PPP_FIXED 8               /* positioning mode: PPP-fixed /ppp定点*/
//解决格式
...
//解决状态
#define SOLQ_NONE   0                   /* solution status: no solution /结果状态：无*/
#define SOLQ_FIX    1                   /* solution status: fix /结果状态：固定解*/
#define SOLQ_FLOAT  2                   /* solution status: float /结果状态：浮点解*/
#define SOLQ_SBAS   3                   /* solution status: SBAS /结果状态：星基增强*/
#define SOLQ_DGPS   4                   /* solution status: DGPS/DGNSS /结果状态：差分*/
#define SOLQ_SINGLE 5                   /* solution status: single /结果状态：单点*/
#define SOLQ_PPP    6                   /* solution status: PPP /结果状态：精密ppp*/
#define SOLQ_DR     7                   /* solution status: dead reconing */
#define MAXSOLQ     7                   /* max number of solution status */
//时间系统
#define TIMES_GPST  0                   /* time system: gps time */
#define TIMES_UTC   1                   /* time system: utc 协调世界时*/
#define TIMES_JST   2                   /* time system: jst 日本标准时间*/
//电离层选项
#define IONOOPT_OFF 0                   /* ionosphere电离层 option: correction off */
#define IONOOPT_BRDC 1                  /* ionosphere option: broadcast model 广播模型*/
#define IONOOPT_SBAS 2                  /* ionosphere option: SBAS model */
#define IONOOPT_IFLC 3                  /* ionosphere option: L1/L2 or L1/L5 iono-free LC */
#define IONOOPT_EST 4                   /* ionosphere option: estimation TEC 估计*/
#define IONOOPT_TEC 5                   /* ionosphere option: IONEX TEC model */
#define IONOOPT_QZS 6                   /* ionosphere option: QZSS broadcast model */
#define IONOOPT_LEX 7                   /* ionosphere option: QZSS LEX ionospehre */
#define IONOOPT_STEC 8                  /* ionosphere option: SLANT TEC model */
//对流层选项
#define TROPOPT_OFF 0                   /* troposphere 对流层option: correction off */
...
//星历表
#define EPHOPT_BRDC 0                   /* ephemeris星历表 option: broadcast ephemeris */
...
//内插模型
#define ARMODE_OFF  0                   /* AR mode: off AR模型是一个类似于插值的类型 */
#define ARMODE_CONT 1                   /* AR mode: continuous */
#define ARMODE_INST 2                   /* AR mode: instantaneous */
#define ARMODE_FIXHOLD 3                /* AR mode: fix and hold */
#define ARMODE_WLNL 4                   /* AR mode: wide lane/narrow lane 宽窄巷*/
#define ARMODE_TCAR 5                   /* AR mode: triple carrier ar 三倍载波AR*/
...
#define SBSOPT_RANGE 8                  /* SBAS option: ranging 距离修正*/
//定位选项
...
#define POSOPT_RINEX 3                  /* pos option: rinex header表头 pos */
#define POSOPT_RTCM  4                  /* pos option: rtcm station pos */
#define POSOPT_RAW   5                  /* pos option: raw station pos */
//流类型
#define STR_NONE     0                  /* stream type: none */
#define STR_SERIAL   1                  /* stream type: serial 连续*/
#define STR_FILE     2                  /* stream type: file 文件*/
#define STR_TCPSVR   3                  /* stream type: TCP server 服务器*/
#define STR_TCPCLI   4                  /* stream type: TCP client 客户*/
...
#define STR_MEMBUF   14                 /* stream type: memory buffer缓冲 */
//流格式
...
#ifndef EXTLEX
#define MAXRCVFMT    15                 /* max number of receiver format接收机格式的最大数目 */
#else
#define MAXRCVFMT    16
#endif

#define STR_MODE_R  0x1                 /* stream mode: read 流的模式 */
...
typedef struct {        /* time struct /时间结构体 单位s 分标准时间和小数部分表示*/
    time_t time;        /* time (s) expressed by standard time_t /标准时间表达时间*/
    double sec;         /* fraction of second under 1 s /s值以下残差*/
} gtime_t;

typedef struct {        /* observation data record /观测文件记录 一个历元一个卫星的结构体*/
    gtime_t time;       /* receiver sampling time (GPST) /接收机采样时间 */
    unsigned char sat,rcv; /* satellite/receiver number /卫星号 接收机号*/
    unsigned char SNR [NFREQ+NEXOBS]; /* signal strength (0.25 dBHz) /信号强度*/
    unsigned char LLI [NFREQ+NEXOBS]; /* loss of lock indicator /失锁标识 */
    unsigned char code[NFREQ+NEXOBS]; /* code indicator (CODE_???) /码类型*/
    double L[NFREQ+NEXOBS]; /* observation data carrier-phase (cycle) /观测数据载波相位*/
    double P[NFREQ+NEXOBS]; /* observation data pseudorange (m) /伪距观测数据*/
    float  D[NFREQ+NEXOBS]; /* observation data doppler frequency (Hz) /多普勒观测数据*/
} obsd_t;

typedef struct {        /* observation data /n个观测数据 */
    int n,nmax;         /* number of obervation data/allocated */
    obsd_t *data;       /* observation data records */
} obs_t;

typedef struct {        /* earth rotation parameter data type 地球自转参数数据类型*/
    double mjd;         /* mjd (days) */
    double xp,yp;       /* pole offset补偿 (rad) */
    double xpr,ypr;     /* pole offset rate (rad/day) */
    double ut1_utc;     /* ut1-utc (s) */
    double lod;         /* length of day (s/day) */
} erpd_t;

typedef struct {        /* earth rotation parameter type 地球自转参数类型*/
    int n,nmax;         /* number and max number of data */
    erpd_t *data;       /* earth rotation parameter data */
} erp_t;

typedef struct {        /* antenna  parameter type天线参数类型 */
    int sat;            /* satellite number 卫星数目(0:receiver) */
    char type[MAXANT];  /* antenna type 最多64个类型，因为最多64台接收机*/
    char code[MAXANT];  /* serial number or satellite code 卫星编码*/
    gtime_t ts,te;      /* valid time start and end 有效起止时间*/
...
typedef struct {        /* almanac历书 type */
    int sat;            /* satellite number 卫星号*/
    int svh;            /* sv health (0:ok) 卫星健康状态*/
...
typedef struct {        /* GPS/QZS/GAL broadcast ephemeris星历表 type /广播星历 */
    int sat;            /* prcopt_t number */
    ...
    double f0,f1,f2;    /* SV clock parameters (af0,af1,af2) 钟参数*/
...
typedef struct {        /* GLONASS broadcast ephemeris type  /glonass广播星历*/
...
typedef struct {        /* precise ephemeris type /精密星历*/
    gtime_t time;       /* time (GPST) */
    int index;          /* ephemeris index for multiple files */
    double pos[MAXSAT][4]; /* satellite position/clock (ecef地固坐标系) (m|s) */
    float  std[MAXSAT][4]; /* satellite position/clock std 卫星和钟的中误差(m|s) */
    double vel[MAXSAT][4]; /* satellite velocity/clk-rate 卫星速度和时钟速率(m/s|s/s) */
    float  vst[MAXSAT][4]; /* satellite velocity/clk-rate std 卫星速度和时钟速率中误差(m/s|s/s) */
    float  cov[MAXSAT][3]; /* satellite position covariance 卫星位置协方差(m^2) */
    float  vco[MAXSAT][3]; /* satellite velocity covariance 卫星速度协方差(m^2) */
} peph_t;

typedef struct {        /* precise clock type /精密钟差*/
    gtime_t time;       /* time (GPST) */
    int index;          /* clock index for multiple files 多种文件时钟索引*/
    double clk[MAXSAT][1]; /* satellite clock (s) 卫星钟*/
    float  std[MAXSAT][1]; /* satellite clock std (s) 卫星钟误差*/
} pclk_t;

typedef struct {        /* SBAS ephemeris type /SBAS星历*/
    int sat;            /* satellite number */
    gtime_t t0;         /* reference epoch time (GPST) 参考纪元时间*/
    gtime_t tof;        /* time of message frame (GPST)消息框架时间 */
    int sva;            /* SV accuracy (URA index) */
    int svh;            /* SV health (0:ok) */
    double pos[3];      /* satellite position (m) (ecef)卫星位置 */
    double vel[3];      /* satellite velocity (m/s) (ecef) 卫星速度*/
    double acc[3];      /* satellite acceleration (m/s^2) (ecef) 卫星加速度*/
    double af0,af1;     /* satellite clock-offset/drift (s,s/s) 卫星钟补差/卫星漂移*/
} seph_t;

typedef struct {        /* norad two line element data type 两线元素数据类型*/
    char name [32];     /* common name */
    char alias[32];     /* alias name别名 */
    char satno[16];     /* satellilte catalog number 卫星目录数目 */
    char satclass;      /* classification分类 */
    char desig[16];     /* international designator国际指示 */
    gtime_t epoch;      /* element set epoch (UTC) */
    double ndot;        /* 1st derivative of mean motion平均运动的一阶导数 */
    double nddot;       /* 2st derivative of mean motion二阶导数 */
...
typedef struct {        /* TEC grid type 电离层延迟类型*/
    gtime_t time;       /* epoch time (GPST) */
    int ndata[3];       /* TEC grid data size {nlat,nlon,nhgt} */
    double rb;          /* earth radius (km) 地球半径*/
...
typedef struct {        /* satellite fcb data type卫星宽巷小数周偏差数据类型 */
...
typedef struct {        /* QZSS LEX ephemeris type */
    gtime_t toe;        /* epoch 纪元 time (GPST) */
    gtime_t tof;        /* message frame time (GPST) */
    int sat;            /* satellite number */
...
typedef struct {        /* ppp corrections type改正模型 */
    int nsta;           /* number of stations 测站数目*/
...
typedef struct {        /* navigation data type /导航电文数据 */
    int n,nmax;         /* number of broadcast ephemeris /broadcast广播星历数量*/
    int ng,ngmax;       /* number of glonass ephemeris /glonass广播星历数量 */
    int ns,nsmax;       /* number of sbas ephemeris /sbas广播星历数量 */
    int ne,nemax;       /* number of precise ephemeris /精密星历数量 */
    int nc,ncmax;       /* number of precise clock /精密钟差数量*/
    int na,namax;       /* number of almanac data /历书 年鉴数*/
    int nt,ntmax;       /* number of tec grid data 电离层延迟网格数据数目*/
    int nf,nfmax;       /* number of satellite fcb data 卫星小数周偏差*/
    eph_t *eph;         /* GPS/QZS/GAL ephemeris 星历 */
    geph_t *geph;       /* GLONASS ephemeris */
    seph_t *seph;       /* SBAS ephemeris */
    peph_t *peph;       /* precise ephemeris 精密星历*/
    pclk_t *pclk;       /* precise clock 精密钟*/
    alm_t *alm;         /* almanac data 历书数据*/
    tec_t *tec;         /* tec grid data */
    fcbd_t *fcb;        /* satellite fcb data */
    erp_t  erp;         /* earth rotation parameters 地球自转参数*/
    ...
    int leaps;          /* leap seconds 跳秒(s) */
    double lam[MAXSAT][NFREQ]; /* carrier wave lengths (m) /载波波长 maxsat*3*/
    double cbias[MAXSAT][3]; /* satellite dcb (0:p1-p2,1:p1-c1,2:p2-c2) 卫星码偏差(m) */
    double rbias[MAXRCV][2][3]; /* receiver dcb (0:p1-p2,1:p1-c1,2:p2-c2)接收机码偏差 (m) */
    double wlbias[MAXSAT];   /* wide-lane bias (cycle) /宽巷偏差*/
...
typedef struct {        /* station parameter type 测站参数类型*/
    char name   [MAXANT]; /* marker name记录名 */
    char marker [MAXANT]; /* marker number 记录号*/
    char antdes [MAXANT]; /* antenna descriptor 天线描述符*/
    char antsno [MAXANT]; /* antenna serial number天线连续数 */
    char rectype[MAXANT]; /* receiver type descriptor 接收机描述符*/
    char recver [MAXANT]; /* receiver firmware version 接收机硬件版本*/
    char recsno [MAXANT]; /* receiver serial number 接收机连续数*/
    int antsetup;       /* antenna setup id 天线安装id*/
    int itrf;           /* ITRF realization year 国际地球参考框架年份*/
    int deltype;        /* antenna delta type (0:enu,1:xyz) 天线位置表达格式*/
    double pos[3];      /* station position (ecef) (m)地固坐标系下测站坐标 */
    double del[3];      /* antenna position delta (e/n/u or x/y/z) (m) 天线坐标*/
    double hgt;         /* antenna height (m) 天线高*/
} sta_t;

typedef struct {        /* solution type解算类型  接收机 */
    gtime_t time;       /* time (GPST) */
    double rr[6];       /* position/velocity (m|m/s)三维坐标 坐标改正数  {x,y,z,vx,vy,vz} or {e,n,u,ve,vn,vu} */
    float  qr[6];       /* position variance/covariance (m^2) 方差协方差*/
                        /* {c_xx,c_yy,c_zz,c_xy,c_yz,c_zx} or */
                        /* {c_ee,c_nn,c_uu,c_en,c_nu,c_ue} */
    double dtr[6];      /* receiver clock bias to time systems (s)接收机钟差 */
    unsigned char type; /* type (0:xyz-ecef,1:enu-baseline)解算类型 笛卡尔 站心 */
    unsigned char stat; /* solution status (SOLQ_???)/ 求解类型 单点 浮点 ppp 固定解等 */
    unsigned char ns;   /* number of valid satellites可见卫星数 */
...
typedef struct {        /* solution buffer type */
    ...
    int cyclic;         /* cyclic buffer flag 循环缓冲区的标志*/
    int start,end;      /* start/end index 开始/结束索引*/
    gtime_t time;       /* current solution time 当前解算时间*/
    sol_t *data;        /* solution data 解算数据*/
    double rb[3];       /* reference position {x,y,z} (ecef)参考位置 (m) */
    unsigned char buff[MAXSOLMSG+1]; /* message buffer */
    int nb;             /* number of byte in message buffer */
} solbuf_t;

typedef struct {        /* solution status type 结算状态*/
    gtime_t time;       /* time (GPST) */
    unsigned char sat;  /* satellite number卫星号 */
    unsigned char frq;  /* frequency (1:L1,2:L2,...) 频率*/
    float az,el;        /* azimuth/elevation angle (rad) 方位角/高度角*/
    float resp;         /* pseudorange residual (m) 伪距残差*/
    float resc;         /* carrier-phase residual (m)载波相位残差 */
    unsigned char flag; /* flags: (vsat<<5)+(slip<<3)+fix */
    unsigned char snr;  /* signal strength (0.25 dBHz)信号强度 */
    unsigned short lock;  /* lock counter 锁计数器*/
    unsigned short outc;  /* outage counter 断开计数器*/
    unsigned short slipc; /* slip counter 平滑计数器*/
    unsigned short rejc;  /* reject counter 拒绝计数器*/
} solstat_t;
...
typedef struct {        /* processing options type /处理选项类型 解算方式 系统 组合 模型等。。。。 -----*/
    int mode;           /* positioning mode (PMODE_???) /定位模式 */
    int soltype;        /* solution type (0:forward,1:backward,2:combined) /解算类型 */
    int nf;             /* number of frequencies (1:L1,2:L1+L2,3:L1+L2+L5) /频率个数 */
    int navsys;         /* navigation system /导航系统 */
    double elmin;       /* elevation mask angle (rad) /截止高度角 */
    snrmask_t snrmask;  /* SNR mask /信噪比限定？ */
    int sateph;         /* satellite ephemeris/clock (EPHOPT_???) /卫星星历 */
    int modear;         /* AR mode (0:off,1:continuous,2:instantaneous,3:fix and hold,4:ppp-ar) */
    int glomodear;      /* GLONASS AR mode (0:off,1:on,2:auto cal,3:ext cal) */
    int bdsmodear;      /* BeiDou AR mode (0:off,1:on) */
    int maxout;         /* obs outage count to reset bias /中断计数重置偏置 */
    int minlock;        /* min lock count to fix ambiguity /修正模糊度最小锁定计数*/
    int minfix;         /* min fix count to hold ambiguity */
    int armaxiter;      /* max iteration to resolve ambiguity 模糊度最大的重复解决数*/
    int ionoopt;        /* ionosphere option (IONOOPT_???) */
    int tropopt;        /* troposphere option (TROPOPT_???) */
    int dynamics;       /* dynamics model (0:none,1:velociy,2:accel) */
    int tidecorr;       /* earth tide correction (0:off,1:solid,2:solid+otl+pole) */
    int niter;          /* number of filter iteration 滤波器迭代次数*/
    int codesmooth;     /* code smoothing window size 码平滑窗口大小 (0:none) */
    int intpref;        /* interpolate reference obs (for post mission)为事后任务插入参考观测值 */
    int sbascorr;       /* SBAS correction options */
    int sbassatsel;     /* SBAS satellite selection (0:all) */
    int rovpos;         /* rover position for fixed mode 固定位置漫游者位置*/
    int refpos;         /* base position for relative mode 相对模型下的基准位置*/
                        /* (0:pos in prcopt,  1:average of single pos, */
                        /*  2:read from file, 3:rinex header, 4:rtcm pos) */
    double eratio[NFREQ]; /* code/phase error ratio 码/相位误差的Ratio值*/
    double err[5];      /* measurement error factor 测量误差误差因素*/
                        /* [0]:reserved */
                        /* [1-3]:error factor a/b/c of phase (m) */
                        /* [4]:doppler frequency (hz) */
    double std[3];      /* initial-state std [0]bias,[1]iono [2]trop */
    double prn[6];      /* process-noise std [0]bias,[1]iono [2]trop [3]acch [4]accv [5] pos 处理噪声的标准差 */
    double sclkstab;    /* satellite clock stability (sec/sec)卫星时钟的稳定性 */
    double thresar[8];  /* AR validation threshold 插值确认门槛*/
    double elmaskar;    /* elevation mask of AR for rising satellite (deg)上升卫星掩盖的高度 */
    double elmaskhold;  /* elevation mask to hold ambiguity (deg) 模糊度掩盖的高度*/
    double thresslip;   /* slip threshold of geometry-free phase (m) /默认值0.05*/
    double maxtdiff;    /* max difference of time (sec) 最大时间差值*/
    double maxinno;     /* reject threshold of innovation (m) */
    double maxgdop;     /* reject threshold of gdop 最大几何精度因子阈值*/
    double baseline[2]; /* baseline length constraint {const,sigma} (m) 基线长度限制*/
    double ru[3];       /* rover position for fixed mode {x,y,z} (ecef) (m) */
    double rb[3];       /* base position for relative mode {x,y,z} (ecef) (m) */
    char anttype[2][MAXANT]; /* antenna types {rover,base} 天线类型*/
    double antdel[2][3]; /* antenna delta {{rov_e,rov_n,rov_u},{ref_e,ref_n,ref_u}} */
    pcv_t pcvr[2];      /* receiver antenna parameters {rov,base} */
    unsigned char exsats[MAXSAT]; /* excluded satellites (1:excluded,2:included) */
    int  maxaveep;      /* max averaging epoches 最大历元平均*/
    int  initrst;       /* initialize by restart */
    int  outsingle;     /* output single by dgps/float/fix/ppp outage */
    char rnxopt[2][256]; /* rinex options {rover,base} */
    int  posopt[6];     /* positioning options 定位选项*/
    int  syncsol;       /* solution sync mode (0:off,1:on)结算同步模型 */
    double odisp[2][6*11]; /* ocean tide loading parameters {rov,base}海洋潮汐加载参数 */
    exterr_t exterr;    /* extended receiver error model */
    int freqopt;        /* disable L2-AR */
    char pppopt[256];   /* ppp option单点定位选项 */
} prcopt_t;

typedef struct {        /* solution options type 解算选项类型*/
    int posf;           /* solution format (SOLF_???)解算格式 */
    int times;          /* time system (TIMES_???)时间系统 */
    int timef;          /* time format时间格式 (0:sssss.s,1:yyyy/mm/dd hh:mm:ss.s) */
    int timeu;          /* time digits under decimal point 在时间点位的时间指数*/
    int degf;           /* latitude/longitude format 经纬度格式(0:ddd.ddd,1:ddd mm ss) */
    int outhead;        /* output header 输出表头(0:no,1:yes) */
    int outopt;         /* output processing options 输出处理选项(0:no,1:yes) */
    int datum;          /* datum (0:WGS84,1:Tokyo) */
    int height;         /* height 高度(0:ellipsoidal,1:geodetic) */
    int geoid;          /* geoid model 大地水准面模型(0:EGM96,1:JGD2000) */
    int solstatic;      /* solution of static mode 静态模型解算(0:all,1:single) */
    int sstat;          /* solution statistics level (0:off,1:states,2:residuals) */
    int trace;          /* debug trace level (0:off,1-5:debug) */
    double nmeaintv[2]; /* nmea output interval 输出区间(s) (<0:no,0:all) */
...
typedef struct {        /* satellite status type /卫星状态类型*/
    unsigned char sys;  /* navigation system /卫星导航系统*/
    unsigned char vs;   /* valid satellite flag single /有效卫星标志？ */
    double azel[2];     /* azimuth/elevation angles {az,el} (rad)/方位角 /高度角数组 */
    double resp[NFREQ]; /* residuals of pseudorange (m) /伪距残差*/
    double resc[NFREQ]; /* residuals of carrier-phase (m) /载波相位残差 */
    unsigned char vsat[NFREQ]; /* valid satellite flag /有效卫星标志*/
    unsigned char snr [NFREQ]; /* signal strength (0.25 dBHz) /信号长度*/
    unsigned char fix [NFREQ]; /* ambiguity fix flag (1:fix,2:float,3:hold) /整周数解标志*/
    unsigned char slip[NFREQ]; /* cycle-slip flag  /周跳标志*/
    unsigned char half[NFREQ]; /* half-cycle valid flag有效半周期 */
    int lock [NFREQ];   /* lock counter of phase相位锁计数器 */
    unsigned int outc [NFREQ]; /* obs outage counter of phase /相位中断计数器*/
    unsigned int slipc[NFREQ]; /* cycle-slip counter 周跳计数器*/
    unsigned int rejc [NFREQ]; /* reject counter拒绝计数器 */
    double  gf;         /* geometry-free phase L1-L2 (m) /无几何影响组合L1-L2*/
    double  gf2;        /* geometry-free phase L1-L5 (m) /无几何影响组合*/
    double  mw;         /* MW-LC (m) */
    double  phw;        /* phase windup (cycle) 相位缠绕*/
    gtime_t pt[2][NFREQ]; /* previous carrier-phase time前载波相位时间 */
    double  ph[2][NFREQ]; /* previous carrier-phase observable前观测得到的周期数 (cycle) */
} ssat_t;

typedef struct {        /* ambiguity control type模糊度控制类型 */
    gtime_t epoch[4];   /* last epoch最后历元 */
    int n[4];           /* number of epochs历元数目 */
    double LC [4];      /* linear combination average平均线性组合 */
    double LCv[4];      /* linear combination variance 线性组合方差*/
    int fixcnt;         /* fix count 固定数*/
    char flags[MAXSAT]; /* fix flags 固定标志：0为固定标志*/
} ambc_t;

typedef struct {        /* RTK control/result type 实时动态定位技术控制*/
    sol_t  sol;         /* RTK solution /求解办法选项*/
    double rb[6];       /* base position/velocity (ecef) (m|m/s) /基本位置 速率*/
    int nx,na;          /* number of float states/fixed states /浮点数/固定状态数 */
    double tt;          /* time difference between current and previous (s) /时间差分 上一个历元与当前历元 */
    double *x, *P;      /* float states and their covariance /浮点解和协方差 长度=9？？？（参考udpos函数）*/
    double *xa,*Pa;     /* fixed states and their covariance /固定解和协方差*/
    int nfix;           /* number of continuous fixes of ambiguity连续固定模糊度数目 */
    ambc_t ambc[MAXSAT]; /* ambibuity control /模糊度控制*/
    ssat_t ssat[MAXSAT]; /* satellite status /卫星状态*/
    int neb;            /* bytes in error message buffer /错误字节消息*/
    char errbuf[MAXERRMSG]; /* error message buffer /错误信息*/
    prcopt_t opt;       /* processing options /解算选项*/
} rtk_t;

typedef struct half_cyc_tag {  /* half-cycle correction list type半周矫正 */
    unsigned char sat;  /* satellite number 卫星号 */
    unsigned char freq; /* frequency number (0:L1,1:L2,2:L5) 频率数*/
    unsigned char valid; /* half-cycle valid flag 有效半周标志*/
    char corr;          /* half-cycle corrected (x 0.5 cyc) */
    gtime_t ts,te;      /* time start, time end 开始与结束时间*/
    struct half_cyc_tag *next; /* pointer to next correction指向下一个改正 */
} half_cyc_t;

typedef struct {        /* receiver raw data control type接收机未处理数据控制 */
    gtime_t time;       /* message time 消息事件 */
    gtime_t tobs;       /* observation data time 观测数据时间*/
    obs_t obs;          /* observation data 观测数据 */
    obs_t obuf;         /* observation data buffer 观测数据缓存*/
    nav_t nav;          /* satellite ephemerides 卫星星历*/
    sta_t sta;          /* station parameters 测站参数*/
    int ephsat;         /* sat number of update ephemeris 星历更新卫星号(0:no satellite) */
    sbsmsg_t sbsmsg;    /* SBAS message */
    char msgtype[256];  /* last message type */
    unsigned char subfrm[MAXSAT][380];  /* subframe buffer */
    lexmsg_t lexmsg;    /* LEX message */
    double lockt[MAXSAT][NFREQ+NEXOBS]; /* lock time 锁时间(s) */
    ...
    int format;         /* receiver stream format接收机流格式 */
    void *rcv_data;     /* receiver dependent data 接收机依赖数据*/
} raw_t;
...
/* coordinates transformation ------------------------------------------------坐标转换*/
...
/* debug trace functions /主要用于文件跟踪调试信息，与应用无关-----------------------------------------*////跟踪？？？？？？？？？？？？？？？？？？？？？？？？
...
/* positioning models --------------------------------------------------------*////卫星波长 高度角 方位角 平滑？？？？？？？？？？？？？？？？？？？？？？？？
...
/* datum transformation ------------------------------------------------------*////基准变换 位置基准tokyo2jgd？？？？？？？？？？？？？？？？？？？？？？
...
/* receiver raw data functions -----------------------------------------------*///接收机原始数据函数
...
/* integer ambiguity resolution ----------------------------------------------*/////整周模糊度
