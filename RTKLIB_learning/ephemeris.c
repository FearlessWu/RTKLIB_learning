...
extern double eph2clk(gtime_t time, const eph_t *eph)
{
    ...
    for (i=0;i<2;i++) {
        t-=eph->f0+eph->f1*t+eph->f2*t*t;//钟差二次方拟合？？？迭代两次
    }
                  ...
                  double *dts)
{
    ...
    sys=satsys(sat,NULL);//得到卫星系统号码
    
    if (sys==SYS_GPS||sys==SYS_GAL||sys==SYS_QZS||sys==SYS_CMP) {
        if (!(eph=seleph(teph,sat,-1,nav))) return 0;//选择合适星历
        *dts=eph2clk(time,eph);//获取卫星钟差
    }
    else if (sys==SYS_GLO) {
...
{
    const sbssatp_t *sbs;
	sbs = (sbssatp_t *)malloc(sizeof(sbssatp_t));
    int i;
...
extern int satpos(gtime_t time, gtime_t teph, int sat, int ephopt,
                  ...
                  int *svh)//卫星的位置
 {
    ...
    *svh=0;
    ...
    switch (ephopt) {//得到卫星的位置以及速度，卫星钟差以及clock drifts
        ...
        case EPHOPT_PREC  :
            if (!peph2pos(time,sat,nav,1,rs,dts,var)) break; else return 1;//得到卫星的位置以及速度，卫星钟差以及clock drifts
        case EPHOPT_LEX   :
...
* args   : gtime_t teph     I   time to select ephemeris (gpst)距选择的星历时间差
*          obsd_t *obs      I   observation data				观测数据
*          int    n         I   number of observation data		观测数据数量
*          nav_t  *nav      I   navigation data					导航电文
*          int    ephopt    I   ephemeris option (EPHOPT_???)	星历选项
*          double *rs       O   satellite positions and velocities (ecef)卫星位置和速度
*          double *dts      O   satellite clocks				卫星钟差
*          double *var      O   sat position and clock error variances (m^2)	卫星位置钟差的方差
*          int    *svh      O   sat health flag (-1:correction not available)	卫星健康标志-1不可用
* return : none
                    ...
                    int ephopt, double *rs, double *dts, double *var, int *svh)
{
    ...
    for (i=0;i<n&&i<2*MAXOBS;i++) {
        for (j=0;j<6;j++) rs [j+i*6]=0.0;//rs--卫星的速度和位置
        for (j=0;j<2;j++) dts[j+i*2]=0.0;//卫星钟差
        var[i]=0.0; svh[i]=0;
        //var--卫星钟差方差；svh--卫星健康状况：-1为不健康

        /* search any psuedorange 查找伪距*/
        ...
        /* transmission time by satellite clock 卫星钟的开始传输的时间 */
        time[i]=timeadd(obs[i].time,-pr/CLIGHT);//其格式时gtime_t格式
        
        /* satellite clock bias by broadcast ephemeris 广播星历的卫星钟差 */
        if (!ephclk(time[i],teph,obs[i].sat,nav,&dt)) {
            trace(3,"no broadcast clock %s sat=%2d\n",time_str(time[i],3),obs[i].sat);
            continue;
        }//广播星历下的卫星钟差
        time[i]=timeadd(time[i],-dt);//dt--卫星钟差
        
        /* satellite position and clock at transmission time 传输时间的卫星位置、速度和卫星钟差 */
        if (!satpos(time[i],teph,obs[i].sat,ephopt,nav,rs+i*6,dts+i*2,var+i,
                    svh+i)) {
            trace(3,"no ephemeris %s sat=%2d\n",time_str(time[i],3),obs[i].sat);
            continue;
        }//得到卫星的位置以及速度，卫星钟差以及clock drifts
      
        /* if no precise clock available, use broadcast clock instead 如果没有可用的精密时钟，用广播星历的钟代替 */
        if (dts[i*2]==0.0) {
