...
* convert satellite id to satellite number 将卫星id转换到卫星号码
...
extern int satexclude(int sat, int svh, const prcopt_t *opt)
{
    ...
    if (svh<0) return 1; /* ephemeris unavailable星历不可用 */
...
/* euclid norm /欧几里得范数 sqrt（AT*A）-----------------------------------------------------------------
* euclid norm of vector
* args   : double *a        I   vector a (n x 1)
*          int    n         I   size of vector a
* return : || a ||
*-----------------------------------------------------------------------------*/
extern double norm(const double *a, int n)//平方和开方
{
    return sqrt(dot(a,a,n));
}

/* outer product of 3d vectors  /x = y1*z2-z1*y2-------------------------------------------------
* outer product of 3d vectors	/y = z1*x2-x1*z2
*								/z = x1*y2-y1*x2
...
/* normalize 3d vector b[0,1,2] = a[0]/sqrt（AT*A）---------------------------------------------------------
* normalize 3d vector
* args   : double *a        I   vector a (3 x 1)
*          double *b        O   normlized vector (3 x 1) || b || = 1
* return : status (1:ok,0:error)
*-----------------------------------------------------------------------------*/
extern int normv3(const double *a, double *b)//计算方向余弦
{
    double r;
...
extern int filter(double *x, double *P, const double *H, const double *v,
                  const double *R, int n, int m)//x--未知数近似值；P--未知数方差；n--nx；m--nv;
{
    double *x_,*xp_,*P_,*Pp_,*H_;
    int i,j,k,info,*ix;
    
    ix=imat(n,1); for (i=k=0;i<n;i++) if (x[i]!=0.0&&P[i+i*n]>0.0) ix[k++]=i;//k=0先执行ix[]=i然后执行k++；最后k=n；
...
extern void time2epoch(gtime_t t, double *ep)
{
    ...
    /* leap year if year%4==0 in 1901-2099 */
    days=(int)(t.time/86400);//将天化为秒为单位
    sec=(int)(t.time-(time_t)days*86400);//溢出秒数
...
*-----------------------------------------------------------------------------*/
extern double timediff(gtime_t t1, gtime_t t2)//获取当前时间
{
...
extern void time2str(gtime_t t, char *s, int n)
{
    ...
    if (1.0-t.sec<0.5/pow(10.0,n)) {t.time++; t.sec=0.0;};//当不足1秒值少于阈值，可向前加一秒；time为标准时间表达
    time2epoch(t,ep);
	/* gps time to time ------------------------------------------------------------
	* convert week and tow in gps time to gtime_t struct
	* args   : int    week      I   week number in gps time
	...
	* return : gtime_t struct*/
    sprintf(s,"%04.0f/%02.0f/%02.0f %02.0f:%02.0f:%0*.*f",ep[0],ep[1],ep[2],
            ep[3],ep[4],n<=0?2:n+3,n<=0?0:n,ep[5]);//输出格式为yyyyMMddHHmmss.sssssssss   s用于存储标准时间
}
/* get time string 获取时间字符串-------------------------------------------------------------
* get time string
* args   : gtime_t t        I   gtime_t struct
*          int    n         I   number of decimals 小数
* return : time string
...
*          double *E        O   ecef to local coord transformation matrix (3x3) 转换矩阵
* return : none
* notes  : matirix stored by column-major order (fortran convention)
*-----------------------------------------------------------------------------*/
...
* transform local enu covariance to xyz-ecef coordinate /变换 地方站心协方差 到 大地坐标系协方差
* args   : double *pos      I   geodetic position {lat,lon} (rad) 大地坐标
*          double *Q        I   covariance in local enu coordinate 站心的协方差
*          double *P        O   covariance in xyz-ecef coordinate xyz的协方差
* return : none
*-----------------------------------------------------------------------------*/
...
* return : status (1:ok,0:error)
*-----------------------------------------------------------------------------*/
extern int geterp(const erp_t *erp, gtime_t time, double *erpv)//获得地球自转参数
{
...
* replace keywords in file path with date, time, rover and base station id
* generate multiple keywords-replaced paths  /生成多个关键字替换路径
* args   : char   *path     I   file path (see below)
...
*-----------------------------------------------------------------------------*/
extern double geodist(const double *rs, const double *rr, double *e)//返回值为卫星到接收机加入地球自转参数改正之后的距离。
{
    double r;
    int i;
    
    if (norm(rs,3)<RE_WGS84) return -1.0;
    for (i=0;i<3;i++) e[i]=rs[i]-rr[i];//卫星与接收机坐标向量之差
    r=norm(e,3);//卫星到接收机的距离
    for (i=0;i<3;i++) e[i]/=r;//获得接收机到卫星的方向余弦；
    return r+OMGE*(rs[0]*rr[1]-rs[1]*rr[0])/CLIGHT;//OMGE为地球自转速度；
}
/* satellite azimuth/elevation angle /求卫星高度角,方位角 -------------------------------------------
...
extern double satazel(const double *pos, const double *e, double *azel)
{
    ...
    if (pos[2]>-RE_WGS84) {
        ecef2enu(pos,e,enu);//接收机转换大地坐标到站心坐标
...
/* ionosphere model /广播星历参数计算电离层延迟 Klobuchar模型------------------------------------------------------------
...
/* ionosphere mapping function /电离层单层模型 利用BLH Ele azi求解-------------------------------------------------
...
/* ionospheric pierce point position /电离层穿透点（IPP）位置和倾斜因子-------------------------------------------
...
/* troposphere model /对流层模型计算 saastamoinen 模型-----------------------------------------------------------
...
/* troposphere mapping function 利用NMF计算对流层映射函数------------------------------------------------
...
*          double *dant     O   range offsets for each frequency (m)对于每个频率偏移范围（m）
* return : none
...
/* sun and moon position in eci 地心坐标系，不与地球旋转(ref [4] 5.1.1, 5.2.1) -----------------------*/
static void sunmoonpos_eci(gtime_t tut, double *rsun, double *rmoon)
{
    ...
    t=timediff(tut,epoch2time(ep2000))/86400.0/36525.0;//t为J2000起算的世纪数；epoch2time把gpst转换成calender day format：ymdhms
    
    /* astronomical arguments天文参数 */
    ast_args(t,f);
    
    /* obliquity of the ecliptic */
    eps=23.439291-0.0130042*t;//黄赤夹角
    sine=sin(eps*D2R); cose=cos(eps*D2R);
    
    /* sun position in eci */
    if (rsun) {
        Ms=357.5277233+35999.05034*t;//太阳平近点角
        ls=280.460+36000.770*t+1.914666471*sin(Ms*D2R)+0.019994643*sin(2.0*Ms*D2R);//太阳几何平黄经
        rs=AU*(1.000140612-0.016708617*cos(Ms*D2R)-0.000139589*cos(2.0*Ms*D2R));//日地距离
        sinl=sin(ls*D2R); cosl=cos(ls*D2R);
        rsun[0]=rs*cosl;
        rsun[1]=rs*cose*sinl;
        rsun[2]=rs*sine*sinl;//太阳在地心惯性坐标系下的坐标
	...
	//obs[0].time--接收机采样时间；erpv--地球自转参数；rsun--太阳地固坐标坐标
{
    ...
    tut=timeadd(tutc,erpv[2]); /* utc -> ut1 指的是世界时与协调世界时之差；此函数获得世界时下的接收机采样时间*/
    
    /* sun and moon position in eci 坐标原点在地球中心，x轴指向春分点，不随着地球自转 */
    sunmoonpos_eci(tut,rsun?rs:NULL,rmoon?rm:NULL);//获得太阳和月球在eci的位置坐标
