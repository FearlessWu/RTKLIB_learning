...
#define NF(opt)     ((opt)->ionoopt==IONOOPT_IFLC?1:(opt)->nf) /* ionosphere option: L1/L2 or L1/L5 iono-free LC */
#define NP(opt)     ((opt)->dynamics==0?3:9)
#define NI(opt)     ((opt)->ionoopt!=IONOOPT_EST?0:MAXSAT)//是否将电离层作为参数估计
#define NT(opt)     ((opt)->tropopt<TROPOPT_EST?0:((opt)->tropopt<TROPOPT_ESTG?2:6))//对流层的选项
#define NL(opt)     ((opt)->glomodear!=2?0:NFREQGLO)
#define NB(opt)     ((opt)->mode<=PMODE_DGPS?0:MAXSAT*NF(opt))//单点定位为0
...
* close solution status file /关闭状态文件 输出等级为0 
* args   : none
* return : none
*-----------------------------------------------------------------------------*/
...
/* write solution status to buffer /将求解结果写入缓冲区------------------------------------------*/
extern int rtkoutstat(rtk_t *rtk, char *buff)
{
    ...
    /* write ppp solution status to buffer /如果模式是ppp的解算模式，使用ppp函数输出结果*/
    ...
    /* receiver position */
    if (est) {
		//012小于固定解数量 就输出固定解坐标，否则固定解结果为0 为什么输出三次？？？
    ...
    /* receiver velocity and acceleration /接收机速度和加速度 动态求解*/
...
/* swap solution status file /解算文件状态交换（基本无用，用来检查error）----------------------------*/
...
/* output solution status /内部静态函数 供输出结果使用 ----------------------------------------------------*/
...
/* save error message /内部静态函数 供输出错误信息使用-------------------------------------------------*/
...
/* single-differenced observable /单差观测值---------------------------------------------
*i 历元 i
*j 历元 j
*f 求差的载波相位位置 0 1 2 
*return : Li-Lj | Pi(f-3) - Pj(f-3)
*------------------------------------------------------------------------------------*/
...
*单差 相位无几何影响组合 lam1*L1i - lam2*L2i -( lam1*L1j - lam2*L2j) 结果是啥？？？ 为了探测周跳
*obs	观测数据
*i		历元i
*j		历元j
*lam	波长
...
/* single-differenced measurement error variance /单差误差方差-----------------------------*/
...
/* baseline length /基线长度 2个坐标的距离 即欧几里得距离-----------------------------------------------------------*/
...
/* initialize state and covariance /初始化rtk的坐标位置和协方差 
*xi 坐标数值
*i  012 坐标是xyz
*------------------------------------------*/
...
/* select common satellites between rover and reference station 一个历元内，在流动站和参考站之间选择共同的卫星 
*azel	高度角信息
*nu		流动站数据数量
*nr		参考站数据数量
*sat	共同的卫星编号 char
                  ...
                  const prcopt_t *opt, int *sat, int *iu, int *ir)
{
    ...
    for (i=0,j=nu;i<nu&&j<nu+nr;i++,j++) {
        ...
        else if (azel[1+j*2]>=opt->elmin) { /* 大于等于截止高度角赋值操作 elevation at base station 基准站高度角*/
...
static void udpos(rtk_t *rtk, double tt)
{
    ...
    /* process noise added to only acceleration /过程噪声只增加加速度 */
...
/* temporal update of ionospheric parameters /电离层参数的时间更新
* 还是在大片更新协方差和坐标
*---------------------------------*/
...
/* temporal update of receiver h/w biases 接收机的什么偏差？？？？？？------------------------------------*/
...
/* detect cycle slip by LLI /探测周跳通过LLI /包括半周周跳--------------------------------------------------*/
...
/* detect cycle slip by L1-L2 geometry free phase jump /L12无几何影响探测周跳-----------------------*/
...
/* detect cycle slip by L1-L5 geometry free phase jump /L1-5探测-----------------------*/
...
/* detect cycle slip by doppler and phase difference /利用多普勒和相位差检测周跳 ------*/
static void detslp_dop(rtk_t *rtk, const obsd_t *obs, int i, int rcv,
                       const nav_t *nav)
{
    /* 因时钟跳变而禁用多普勒检测 detection with doppler disabled because of clock-jump issue (v.2.3.0) */
#if 0
    int f,sat=obs[i].sat;
...
/* temporal update of phase biases /相位偏差的时间更新-------------------------------------------*/
                   ...
                   const int *iu, const int *ir, int ns, const nav_t *nav)
{
    ...
    for (f=0;f<nf;f++) {
        /* reset phase-bias if instantaneous AR or expire obs outage counter 重置相位偏置*/
        for (i=1;i<=MAXSAT;i++) {
        ...
        /* estimate approximate phase-bias by phase - code 根据码，相位估计偏差*/
		...
		/* correct phase-bias offset to enssure phase-code coherency 修正码偏差来保证相位测量与码测量的一致性*/
        if (j>0) {
            for (i=1;i<=MAXSAT;i++) {
                if (rtk->x[IB(i,f,&rtk->opt)]!=0.0) rtk->x[IB(i,f,&rtk->opt)]+=offset/j;
            }
        }
        /* set initial states of phase-bias 相位偏置的初始状态*/
        for (i=0;i<ns;i++) {
...
/* temporal update of states 状态的时间更新 ------------------------------
* ns 观测值数
*iu ir 对应的卫星位置
*
*--------------------------------------------------*/
...
/* undifferenced phase/code residuals /先进行距离改正后历元zdres_sat求残差值----------------------------------------*/
                 ...
                 double *e, double *azel)
{
    ...
    /* earth tide correction 对接收机位置进行固体潮改正*/
    if (opt->tidecorr) {
    ...
    for (i=0;i<n;i++) {
        /* compute geometric-range and azimuth/elevation angle 计算几何距离和方位角/仰角*/
        if ((r=geodist(rs+i*6,rr_,e+i*3))<=0.0) continue;
        if (satazel(pos,e+i*3,azel+i*2)<opt->elmin) continue;
        
        /* excluded satellite? 排除不健康卫星*/
        if (satexclude(obs[i].sat,svh[i],opt)) continue;
        
        /* satellite clock-bias /卫星钟差改正 */
        r+=-CLIGHT*dts[i*2];
        
        /* troposphere delay model (hydrostatic)/对流层改正 */
        zhd=tropmodel(obs[0].time,pos,zazel,0.0);
        r+=tropmapf(obs[i].time,pos,azel+i*2,NULL)*zhd;
        
        /* receiver antenna phase center correction /相位中心*/
        antmodel(opt->pcvr+index,opt->antdel[index],azel+i*2,opt->posopt[1],
                 dant);
        
        /* undifferenced phase/code residual for satellite/非差残差改正 */
...
/* test valid observation data /测试有效观测数据
*  return 0 该历元不可使用 无相位观测值 
*-----------------------------------------------*/
...
/* double-differenced measurement error covariance 双差测量误差 协方差
* nb
* n
* Ri
* Rj
...
/* baseline length constraint 基线长度约束
*------------------------------------------------*/
                   ...
                   double *H, double *Ri, double *Rj, int index)
{
    ...
    /* time-adjusted baseline vector and length /时间调整基线向量和长度 */
    for (i=0;i<3;i++) {
...
/* precise tropspheric model 精密对流层模型
*
*-------------------------------------------------*/
                       ...
                       double *dtdx)
{
    ...
    /* wet mapping function 利用NMF计算对流层映射函数*/
...
/* test navi system (m=0:gps/qzs/sbs,1:glo,2:gal,3:bds) 检验导航系统是不是sys ----------------------*/
...
/* double-differenced phase/code residuals  /双差 相位/码 残余
*
*
*
*
                 ...
                 double *H, double *R, int *vflg)
{
	...
	*	计算电离层 对流层延迟因子??? 这个计算公式结果是什么？
	*///azel + ir[i] * 2  //dtdxu + i * 3 这几项干嘛的 为什么是加法
    for (i=0;i<ns;i++) {
    ...
    for (f=opt->mode>PMODE_DGPS?0:nf;f<nf*2;f++) {
        ...
        /* search reference satellite with highest elevation获取流动站的最高卫星高度角卫星下标i */
        for (i=-1,j=0;j<ns;j++) {
            sysi=rtk->ssat[sat[j]-1].sys;
            if (!test_sys(sysi,m)) continue;//如果不是m导航系统 则继续
            if (!validobs(iu[j],ir[j],f,nf,y)) continue;//如果观测值无效 则继续
            if (i<0||azel[1+iu[j]*2]>=azel[1+iu[i]*2]) i=j;//i< 0 或 j卫星高度角大于i卫星 i=j
			//azel[1+iu[j]*2] 意思是 多颗卫星高度角在里面 流动站第j课卫星高度角位置???
        }
        if (i<0) continue;
        
        /* make double difference  做双差差分*/
        for (j=0;j<ns;j++) {
            if (i==j) continue;
            ...
            /* partial derivatives by rover position /接收机位置偏导数 */
            if (H) {
                for (k=0;k<3;k++) {
...
/* time-interpolation of residuals (for post-mission) 时间内插的残差（后处理）------------------------*/
...
/* single to double-difference transformation matrix (D')单到双差变换矩阵 --------------------*/
...
/* restore single-differenced ambiguity /恢复单差模糊度-----------------------*/
                  ...
                  int nv, double thres)
{
    ...
    /* post-fit residual test 拟合后残差*/
    for (i=0;i<nv;i++) {
        if (v[i]*v[i]<=fact*R[i+i*nv]) continue; //数据正确的情况 残差不超多少 vv<=threa * thres * R
        sat1=(vflg[i]>>16)&0xFF;
        sat2=(vflg[i]>> 8)&0xFF;
        type=(vflg[i]>> 4)&0xF;
        freq=vflg[i]&0xF;
...
/* relative positioning   /相对定位 nu流动站观测数  nr参考站观测数
*rtk rtk
*obs 一个历元一个卫星结构 前nu个流动站 后nr个流动站
nu	 流动站观测历元数目
nr   参考站观测历元数目
*nav 导航电文
*----------------------------*/
                  ...
                  const nav_t *nav)
{
	...
	//参考站 流动站 第一个历元的时间差值 dt
    dt=timediff(time,obs[nu].time);
    
	/* n   观测历元总数 参考站+流动站
	   rs  6 * n 卫星位置坐标 速度 
	   dts 卫星钟差 2 * n 流动/参考 每一个历元卫星钟差
	   var 卫星位置钟差的方差
	   y   频率乘2 * n
	   e   3 * n 
	*/
	...
	//给rtk -> ssat 初始值
    for (i=0;i<MAXSAT;i++) {
		//给rtk -> ssat 赋值 设置卫星号对应的导航系统
        rtk->ssat[i].sys=satsys(i+1,NULL);
        for (j=0;j<NFREQ;j++) rtk->ssat[i].vsat[j]=0;//有效标志
        for (j=1;j<NFREQ;j++) rtk->ssat[i].snr [j]=0;//信号长度
    }

    /* satellite positions/clocks 
	opt->sateph 星历类型 广播 还是 精密星历 
	time        流动站第一个历元GPS时
	rs dts var svh 作为返回值 位置 钟差 方差 健康标志
	*/
    satposs(time,obs,n,nav,opt->sateph,rs,dts,var,svh);
    
    /* undifferenced residuals for base station /基准站的非差残差 
	*/
...
* return : none
*-----------------------------------------------------------------------------*/
extern void rtkinit(rtk_t *rtk, const prcopt_t *opt)//初始化rtk控制
{
    sol_t sol0={{0}};
    ambc_t ambc0={{{0}}};
    ssat_t ssat0={0};
    ...
    rtk->nx=opt->mode<=PMODE_FIXED?NX(opt):pppnx(opt);//浮点状态数
    rtk->na=opt->mode<=PMODE_FIXED?NR(opt):pppnx(opt);//固定状态数
    rtk->tt=0.0;
    rtk->x=zeros(rtk->nx,1);//浮点解矩阵，初始值为零
    rtk->P=zeros(rtk->nx,rtk->nx);//浮点解协方差矩阵，初始值为零
    rtk->xa=zeros(rtk->na,1);//固定解矩阵
    rtk->Pa=zeros(rtk->na,rtk->na);//固定解协方差
    rtk->nfix=rtk->neb=0;//连续固定数目与错误消息
    for (i=0;i<MAXSAT;i++) {
        rtk->ambc[i]=ambc0;//模糊度初始化
        rtk->ssat[i]=ssat0;//卫星状态的初始化
    }
...
* precise positioning 精密定位 输入观测数据 导航信息 计算接收机位置
* args   : rtk_t *rtk       IO  rtk control/result struct
*            rtk->sol       IO  solution
...
*          int    n         I   number of observation data 观测数目个数
...
extern int rtkpos(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
    ...
    if (opt->refpos<=POSOPT_RINEX&&opt->mode!=PMODE_SINGLE&&
        ...
        for (i=0;i<6;i++) rtk->rb[i]=i<3?opt->rb[i]:0.0;// base station position/velocity基准站的位置和速度
    }
    /* count rover/base station observations 统计流动站 基准站观测数据数目
	*	数据obs前n个是接收机 后面的nr个是基准站的观测数据
	*/
    for (nu=0;nu   <n&&obs[nu   ].rcv==1;nu++) ;//rcv为接收机号
    for (nr=0;nu+nr<n&&obs[nu+nr].rcv==2;nr++) ;//基准站数据
    
    time=rtk->sol.time; /* previous epoch 上个历元的时间 */
    
    /* rover position by single point positioning 单点定位接收机位置*/
    ...
    if (opt->mode>=PMODE_PPP_KINEMA) {//bigger than PMODE_PPP_KINEMA
        pppos(rtk,obs,nu,nav);
        outsolstat(rtk);
        return 1;
    }
    
	/* check number of data of base station and age of differential /检查基站数据数和差分讯龄 */
    if (nr==0) {
