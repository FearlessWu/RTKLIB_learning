...
{
	//判断那种系统 获取误差因子 GPS/GAL/QZS/CMP=1.0 ，GLO/IRN = 1.5 SBAS =3.0
	//
    double fact,varr;
...
/* get tgd parameter (m) /获取sat号卫星的导航电文tgd参数（电离层群延迟改正参数）-----------*/
...
/* psendorange with code bias correction /伪距偏差校正----------------------------*/
...
/* ionospheric correction /电离层改正 根据不同改正选项改正------------------------------------------------------
...
/* tropospheric correction /对流层模型改正 -----------------------------------------------------
                   ...
                   double *resp, int *ns)
{
    ...
    ecef2pos(rr,pos);//转到大地坐标
    
    for (i=*ns=0;i<n&&i<MAXOBS;i++) {
        vsat[i]=0; azel[i*2]=azel[1+i*2]=resp[i]=0.0;//卫星方位角为0；
		//if (nv == 4) continue;
        if (!(sys=satsys(obs[i].sat,NULL))) continue;
		//if (sys != SYS_CMP) continue;
		//if (obs[i].sat < 95) continue;
        /* reject duplicated observation data */
        ...
        if ((r=geodist(rs+i*6,rr,e))<=0.0||//r--地球到卫星的距离
            satazel(pos,e,azel+i*2)<opt->elmin) continue;//求得卫星方位角与高度角
        
        /* psudorange with code bias correction伪距偏差校正 */
		...
		//double  a = varerr(opt, azel[1 + i * 2], sys) + vare[i] + vmeas + vion + vtrp;
        /* design matrix */
        ...
        /* error variance */
        var[nv++]=varerr(opt,azel[1+i*2],sys)+vare[i]+vmeas+vion+vtrp;//f???
		//printf("%f\n", var[nv-1]);
...
static int estpos(const obsd_t *obs, int n, const double *rs, const double *dts,//利用伪距获得接收机的位置坐标
                  ...
                  double *resp, char *msg)
{
    ...
    for (i=0;i<MAXITR;i++) {//最大迭代次数
        
        /* pseudorange residuals */
        nv=rescode(i,obs,n,rs,dts,vare,svh,nav,x,opt,v,H,var,azel,vsat,resp,//n为接收机观测数据数;H v var
                   &ns);
        ...
        if (norm(dx,NX)<1E-4) {
            sol->type=0;
            sol->time=timeadd(obs[0].time,-x[3]/CLIGHT);//时间改正
            ...
            sol->ns=(unsigned char)ns;//可见的卫星数目
                  ...
                  char *msg)
{
    ...
    /* satellite positons, velocities and clocks 卫星位置、速度和时钟*/
    satposs(sol->time,obs,n,nav,opt_.sateph,rs,dts,var,svh);
    
    /* estimate receiver position with pseudorange用伪距估计接收机位置 */
    stat=estpos(obs,n,rs,dts,var,svh,nav,&opt_,sol,azel_,vsat,resp,msg);//n为接收机观测数据的数量。var--卫星位置钟差方差，
    
    /* raim fde */
    if (!stat&&n>=6&&opt->posopt[4]) {
        stat=raim_fde(obs,n,rs,dts,var,svh,nav,&opt_,sol,azel_,vsat,resp,msg);//dts卫星钟差
    }
