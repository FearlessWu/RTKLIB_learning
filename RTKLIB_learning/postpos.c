/*------------------------------------------------------------------------------
* postpos.c : post-processing positioning 后处理定位
*
...
/* output reference position 输出的参考位置-------------------------------------------------*/
...
/* search next observation data index 下一个相同历元的观测数据索引----------------------------------------*/
static int nextobsf(const obs_t *obs, int *i, int rcv)
{
    double tt;
    int n;
    
    for (;*i<obs->n;(*i)++) if (obs->data[*i].rcv==rcv) break;//n--the number of epoch;从i开始找与指定rcv=1号的接收机
    for (n=0;*i+n<obs->n;n++) {//从指定的接收机号往前找下一个接收机使得他们的时间差超过0.005s
        tt=timediff(obs->data[*i+n].time,obs->data[*i].time);//两个接收机采样的时间差
        if (obs->data[*i+n].rcv!=rcv||tt>DTTOL) break;//DTTOL采样时间相差阈值;find a same epoch rcv which is different from the former rcv;
    }
    return n;
}
...
static int inputobs(obsd_t *obs, int solq, const prcopt_t *popt)
{
    ...
    if (0<=iobsu&&iobsu<obss.n) {//n观测数据数目
        settime((time=obss.data[iobsu].time));
        if (checkbrk("processing : %s Q=%d",time_str(time,0),solq)) {
            aborts=1; showmsg("aborted"); return -1;
        }
    }
    if (!revs) { /* input forward data (0:forward,1:backward) */
        if ((nu=nextobsf(&obss,&iobsu,1))<=0) return -1;//nu为当前接收机号到下一个相同历元接收机的序号差
        if (popt->intpref) {//为后处理任务插入观测值
        ...
        while (isbs<sbss.n) {
            time=gpst2time(sbss.msgs[isbs].week,sbss.msgs[isbs].tow);//接收时间转换
                    ...
                    int mode)
{
    ...
    solstatic=sopt->solstatic&&  /* solution of static mode 静态模型解算(0:all,1:single) */
              (popt->mode==PMODE_STATIC||popt->mode==PMODE_PPP_STATIC);//处理选项为静态
    
    rtkinit(&rtk,popt);//rtk控制初始化
    rtcm_path[0]='\0';
    ...
    while ((nobs=inputobs(obs,rtk.sol.stat,popt))>=0) {
        ...
        for (i=n=0;i<nobs;i++) {
            if ((satsys(obs[i].sat,NULL)&popt->navsys)&&
                popt->exsats[obs[i].sat-1]!=1) obs[n++]=obs[i];//排除不可用卫星的观测数据，n成为观测数据的数量
			//i--represent the epoch
        }
        if (n<=0) continue;
        ...
        if (navs.nf>0) {
            ...
            corr_phase_bias_ssr(obs,n,&navs);//n为观测数据的数目；载波相位偏差SSR改正
        }
        /* disable L2 */
#if 0
        if (popt->freqopt==1) {
                        ...
                        nav_t *nav, sbs_t *sbs, lex_t *lex)
{
    ...
    /*initialize */
    nav->ne=nav->nemax=0;//the number of precise ephemeris
    nav->nc=nav->ncmax=0;//the number of precise clock
    nav->nf=nav->nfmax=0;//the number of satellite fcb data
    sbs->n =sbs->nmax =0;//the number of sbas messages/allocted
    lex->n =lex->nmax =0;//the number of lex messages/allocted
    
    /* read precise ephemeris files 
	note: the ephemeris data were stored in structure peph*/
    for (i=0;i<n;i++) {
        if (strstr(infile[i],"%r")||strstr(infile[i],"%b")) continue;
        readsp3(infile[i],nav,0);//0--only observed
    }
                      ...
                      obs_t *obs, nav_t *nav, sta_t *sta)
{
    ...
    obs->data=NULL; obs->n =obs->nmax =0;//initialize
    ...
    for (i=0;i<n;i++) {
        ...
        if (index[i]!=ind) {
            if (obs->n>nobs) rcv++;
            ind=index[i]; nobs=obs->n; //nobs means the number of the observations.
        }
                   ...
                   const filopt_t *fopt, nav_t *nav, pcvs_t *pcvs, pcvs_t *pcvr)
{
    ...
    /* open geoid data */
    if (sopt->geoid>0&&*fopt->geoid) { /* geoid model 大地水准面模型(0:EGM96,1:JGD2000) */
...
/* execute processing session 执行数据解算------------------------------------------------*/
                   ...
                   char **infile, const int *index, int n, char *outfile)
{
    ...
    if (popt_.mode==PMODE_FIXED) {
        ...
        if (!antpos(&popt_,2,&obss,&navs,stas,fopt->stapos)) {//antenna phase center position
            freeobsnav(&obss,&navs);
            return 0;
        }
    }
...
/* execute processing session for each rover 每个接收机数据解算---------------------------------*/
                     ...
                     const char *rov)
{
    ...
    for (i=0;i<n;i++) if (strstr(infile[i],"%r")) break;//the file dosen't include "%r";
    ...
    if (i<n) { /* include rover keywords */
        ...
        for (p=rov_;;p=q+1) { /* for each rover */
            if ((q=strchr(p,' '))) *q='\0';//according to the rcs name
...
/* execute processing session for each base station 每个基站解算--------------------------*/
static int execses_b(gtime_t ts, gtime_t te, double ti, const prcopt_t *popt,
	const solopt_t *sopt, const filopt_t *fopt, int flag,
	char **infile, const int *index, int n, char *outfile,
	const char *rov, const char *base)//n--the number of file 
{
	gtime_t t0 = { 0 };
	int i, stat = 0;
...
* return : status (0:ok,0>:error,1:aborted)
	输入文件应包含观测数据、导航数据、精确数据。
*星历和时钟（可选），SBAS日志文件（可选），SSR的消息
*日志文件（可选）和TEC网格文件（可选）。只有第一
*输入文件中的观测数据文件被识别为接收机的。
                   ...
                   const char *rov, const char *base)
{
	...
	/*1.read satellite antenna parameter
	  2.read recevier antenna paramenter
	  3.open geoid data
	  4.use satellite L2 offset if L5 offset is not avaliable*/
    if (!openses(popt,sopt,fopt,&navs,&pcvss,&pcvsr)) return -1;//this function is not inveled.
    

    if (ts.time!=0&&te.time!=0&&tu>=0.0) {//when the start timr and end time is not null
		/*have no function*/
        if (timediff(te,ts)<0.0) {
            showmsg("error : no period");
            closeses(&navs,&pcvss,&pcvsr);
            return 0;
        }//this function is not involed.

		/*This part is used to allocte memory space for every ifile*/
        for (i=0;i<MAXINFILE;i++) {
            if (!(ifile[i]=(char *)malloc(1024))) {//mallc() is used to allocting memory space. 
		...
		/*get the unit time of processing 
		ags: tunit--unit time of prossing
		     tss--the max int number of tunit in start time which is under a int week*/

        if (tu==0.0||tu>86400.0*MAXPRCDAYS) tu=86400.0*MAXPRCDAYS;//MACPRCDAYS--the max days of continuous processing 
        settspan(ts,te);
        ...
        for (i=0;;i++) { /* for each periods */
            ...
            tte=timeadd(tts,tu-DTTOL);//tts + tu
			...
			/*this round is copy infile to ifile*/
            for (j=k=nf=0;j<n;j++) {//n--the number of raw files;nf--the number of ifile
                
                ext=strrchr(infile[j],'.');
                
                if (ext&&(!strcmp(ext,".rtcm3")||!strcmp(ext,".RTCM3"))) {//PS:rtcm3 is a form of GNSS observation file
                    strcpy(ifile[nf++],infile[j]);//ext is exist and rtcm3 is not exist,then copy the infile to ifile.
                }
                else {
                ...
                while (k<nf) index[k++]=j;//initialize the index=0;
            ...
            stat=execses_b(tts,tte,ti,popt,sopt,fopt,flag,ifile,index,nf,ofile,
                           rov,base);//nf--the number of ifile;base--base station id list
        ...
        for (i=0;i<n;i++) index[i]=i;//n represent number of the file
